...
{
    UE_LOG(LogTemp, Log, TEXT("World Switch %s => %s"), *GetNameSafe(CurrentWorld.Get()), *GetNameSafe(InWorld.Get()));
...
void FSubsystemModel::GetFilteredCategories(TArray<SubsystemTreeItemPtr>& OutCategories)
{
    ...
    for (const SubsystemTreeItemPtr& Item : GetAllCategories())
    {
        ...
        if (USubsystemBrowserSettings::Get()->ShouldHideEmptyCategories() && !GetNumSubsystemsFromCategory(Item))
        {
            continue;
        }
