...
void FSubsystemBrowserModule::StartupModule()
{
    if (GIsEditor && !IsRunningCommandlet())
    {
        ...
        { // register as a nomad tab within global tab manager
            FGlobalTabmanager::Get()->RegisterNomadTabSpawner(SubsystemBrowserNomadTabName, FOnSpawnTab::CreateRaw(this, &FSubsystemBrowserModule::HandleSpawnBrowserTab)).SetDisplayName(LOCTEXT("SubsystemBrowserNomadTabTitle", "Subsystem Browser")).SetTooltipText(LOCTEXT("SubsystemBrowserNomadTabTooltip", "Open the Subsystem Browser tab."))
...
void FSubsystemBrowserModule::RegisterCategory(TSharedRef<FSubsystemCategory> InCategory)
{
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("Invalid category being registered"));
        return;
    }
    ...
    for (const SubsystemCategoryPtr& Category : Categories)
    {
        ...
        {
            UE_LOG(LogTemp, Error, TEXT("Duplicating category with name %s."), *Category->Name.ToString());
            return;
        }
    }
...
void FSubsystemBrowserModule::RegisterDynamicColumn(TSharedRef<FSubsystemDynamicColumn> InColumn)
{
    ...
    {
        UE_LOG(LogTemp, Error, TEXT("Invalid column being registered"));
        return;
    }
    ...
    for (SubsystemColumnPtr& Column : DynamicColumns)
    {
        ...
        {
            UE_LOG(LogTemp, Error, TEXT("Duplicating column with name %s."), *Column->Name.ToString());
            return;
        }
    }
