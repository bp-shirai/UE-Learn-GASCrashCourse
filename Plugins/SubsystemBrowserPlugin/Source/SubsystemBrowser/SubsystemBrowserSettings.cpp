...
{
    UE_LOG(LogTemp, Verbose, TEXT("Browser settings being selected"));
...
{
    UE_LOG(LogTemp, Verbose, TEXT("Browser settings being modified"));
    return true;
}
...
{
    UE_LOG(LogTemp, Verbose, TEXT("Browser settings being reset"));
    ...
    bool bResettable = GetClass()->HasAnyClassFlags(CLASS_Config) && !GetClass()->HasAnyClassFlags(CLASS_DefaultConfig | CLASS_GlobalUserConfig | CLASS_ProjectUserConfig);
...
{
    UE_LOG(LogTemp, Verbose, TEXT("Property %s changed"), *PropertyName.ToString());
