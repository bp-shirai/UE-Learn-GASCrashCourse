                            ...
                            [SAssignNew(BrowserSplitter, SSplitter)
                             ...
                             SSplitter::Slot()
                                 ...
                                 .Value(Settings->GetSeparatorLocation())
                                     [SNew(SVerticalBox) + SVerticalBox::Slot().FillHeight(1.f).Padding(0, 0, 0, 2)[SAssignNew(TreeWidget, SSubsystemsTreeWidget, SubsystemModel, SharedThis(this)).TreeItemsSource(&RootTreeItems).SelectionMode(ESelectionMode::Single).OnGenerateRow(this, &SSubsystemBrowserPanel::GenerateTreeRow).OnGetChildren(this, &SSubsystemBrowserPanel::GetChildrenForTree).OnSelectionChanged(this, &SSubsystemBrowserPanel::OnSelectionChanged).OnExpansionChanged(this, &SSubsystemBrowserPanel::OnExpansionChanged).OnMouseButtonDoubleClick(this, &SSubsystemBrowserPanel::OnTreeViewMouseButtonDoubleClick).OnContextMenuOpening(this, &SSubsystemBrowserPanel::ConstructSubsystemContextMenu).HighlightParentNodesForSelection(true).ClearSelectionOnClick(true).HeaderRow(HeaderRowWidget.ToSharedRef())]
                                      ...
                                      + SVerticalBox::Slot()
                                            ...
                                            .Padding(0, 0, 0, 1)
                                                [SNew(SSeparator)
                                                     .Visibility(EVisibility::Visible)]] +
                             SSplitter::Slot()
                                 [SAssignNew(DetailsViewBox, SVerticalBox) + SVerticalBox::Slot()
                                                                                 .Padding(0, 4, 0, 2)
...
{
    UE_LOG(LogTemp, Log, TEXT("Selected world %s"), *GetNameSafe(InWorld.Get()));
...
{
    UE_LOG(LogTemp, Verbose, TEXT("On PIE Start"));
...
{
    UE_LOG(LogTemp, Verbose, TEXT("On PIE End"));
...
{
    UE_LOG(LogTemp, Verbose, TEXT("On World Changed"));
...
void SSubsystemBrowserPanel::SetSelectedObject(SubsystemTreeItemPtr Item)
{
    ...
    UE_LOG(LogTemp, Log, TEXT("Selected object %s"), *GetNameSafe(InObject));
...
void SSubsystemBrowserPanel::ResetSelectedObject()
{
    ...
    UE_LOG(LogTemp, Log, TEXT("Reset selected object"));
...
TSharedPtr<SWidget> SSubsystemBrowserPanel::ConstructSubsystemContextMenu()
{
    ...
    if (!ToolMenus->IsMenuRegistered(FSubsystemBrowserModule::SubsystemBrowserContextMenuName))
    {
        ...
        UE_LOG(LogTemp, Warning, TEXT("For some reason context menu did not register"));
    }
