...
TOptional<FString> FSubsystemBrowserUtils::GetSmartMetaValue(UObject* InObject, const FName& InName, bool bHierarchical, bool bWarn)
{
    ...
    if (UserSource.IsSet() && FName::IsValidXName(UserSource.GetValue(), INVALID_OBJECTNAME_CHARACTERS))
    {
        ...
        if (bWarn)
        {
            UE_LOG(LogTemp, Warning, TEXT("%s specifies source %s but it is neither valid function or property"),
    ...
    if (InClass)
    {
        if (UPackage* ClassPackage = InClass->GetOuterUPackage())
        {
            ...
            if (FModuleManager::Get().IsModuleLoaded(ShortClassPackageName))
            {
                ...
                {
                    UE_LOG(LogTemp, Warning, TEXT("Failed to resolve module details of %s: Query Failed"), *ShortClassPackageName.ToString());
                }
            }
            else
            {
                UE_LOG(LogTemp, Warning, TEXT("Failed to resolve module details of %s: Not Loaded Yet"), *ShortClassPackageName.ToString());
            }
        }
    }
    return false;
...
FSubsystemBrowserUtils::FClassFieldStats FSubsystemBrowserUtils::GetClassFieldStats(UClass* InClass)
{
    ...
    for (TFieldIterator<UFunction> It(InClass); It; ++It)
    {
        ...
        if (TestFunction->FindMetaData(MD_CallInEditor) != nullptr || TestFunction->FindMetaData(FSubsystemBrowserUserMeta::MD_SBQuickAction) != nullptr)
        {
            Stats.NumCallable++;
        }
    }
...
{
    UE_LOG(LogTemp, Log, TEXT("Clipboard set to:\n%s"), *ClipboardText);
...
void FSubsystemBrowserUtils::DefaultSelectSubsystemSubobjects(UObject* InSubsystem, TArray<UObject*>& OutData)
{
    ...
    else // not empty
    {
        ...
        {
            UE_LOG(LogTemp, Warning, TEXT("Invalid SBGetSubobjects value (%s) for type %s"), *FuncName.ToString(), *InSubsystem->GetClass()->GetName());
        }
...
bool FSubsystemBrowserUtils::TryParseColor(const FString& InColor, FLinearColor& OutColor)
{
    ...
    UE_LOG(LogTemp, Warning, TEXT("Failed to parse color property [%s]"), *StrValue);
    return false;
}
